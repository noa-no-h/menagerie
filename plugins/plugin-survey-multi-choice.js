var jsPsychSurveyMultiChoice = (function (jspsych) {
  ...
  const info = {
    ...
    parameters: {
      ...
      questions: {
        ...
        nested: {
          ...
          /** The correct response for the question. If provided, validation will occur. */
          correct_response: {
            type: jspsych.ParameterType.STRING,
            default: null
          }
    ...
    citations: {
      ...
      "bibtex": '@article{Leeuw2022jsPsych, author = {de Leeuw, Joshua R. and Gilbert, Rebecca A. and Luchterhandt, Bj{\\" o}rn}, journal = {Journal of Open Source Software}, doi = {10.21105/joss.05351}, issn = {2475-9066}, number = {85}, year = {2023}, month = {may 11}, pages = {5351}, publisher = {Open Journals}, title = {jsPsych: Enabling an {Open}-{Source} {Collaborative} {Ecosystem} of {Behavioral} {Experiments}}, url = {https://joss.theoj.org/papers/10.21105/joss.05351}, volume = {8}, }  '
    }
  };
  ...
  class SurveyMultiChoicePlugin {
    ...
    <style id="${plugin_id_name}-css">
      ...
      trial_form.addEventListener("submit", (event) => {
        ...
        var question_data = {};
        let all_answers_correct = true; // Flag to track if all answers are correct

        for (var i2 = 0; i2 < trial.questions.length; i2++) {
          var question_obj = trial.questions[question_order[i2]]; // Get the actual question object
        ...
        if (all_answers_correct) {
          ...
          console.log("not finishing trial");
          // If not all answers are correct, don't finish the trial.
    ...
    create_simulation_data(trial, simulation_options) {
      ...
      for (const q of trial.questions) {
        ...
        // For simulation, if a correct_response is defined, use it. Otherwise, pick a random option.
        question_data[name] = q.correct_response !== null ? q.correct_response : this.jsPsych.randomization.sampleWithoutReplacement(q.options, 1)[0];
    ...
    simulate_visual(trial, simulation_options, load_callback) {
      ...
      for (let i = 0; i < answers.length; i++) {
        // Find the index of the correct answer in the options array for the current question
        const question_index_in_original_array = data.question_order[i]; // Get the original index of the question
        const correct_answer_value = answers[i][1];
        const option_index = trial.questions[question_index_in_original_array].options.indexOf(correct_answer_value);

        if (option_index !== -1) {
          this.jsPsych.pluginAPI.clickTarget(
            display_element.querySelector(
              `#${plugin_id_name}-response-${i}-${option_index}`
            ),
